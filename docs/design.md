...
`ha-switch` 是一个通用的本地 profile / state 切换工具。它把某个本地 App 或 CLI 的一组可管理状态建模为 profile，并通过声明式 App Definition，把 profile 捕获、渲染或回放到具体 target，例如 JSON 子树、TOML 路径、文件、Keychain entry 或环境变量块。

MVP 阶段随二进制内置 Claude Code 和 Codex 的 App Definition，用来覆盖账号、凭据、服务端点和模型配置这些高风险状态切换场景。但这些只是第一组内置用例，不是 core 的边界；同一套模型也服务于其他能被声明式定义描述的本地软件状态。
...
这个问题同时验证 ha-switch 的核心抽象：声明式 target、受信任 handler、防御性备份、失败回滚、secret 脱敏和动态状态写回。

示例：

```bash
# 把当前 Claude Code 的 Anthropic OAuth 登录捕获为一条 profile (oauth_capture)
ha-switch import-current claude anthropic-work

# 添加一个 GLM 第三方代理 profile (env_injection)
ha-switch add claude glm \
  --kind env_injection \
  --field base_url=https://open.bigmodel.cn/api/anthropic \
  --secret-field auth_token=@stdin
# 从 stdin 读取 token，避免 token 进入 shell history / argv

# 切到 GLM 代理
ha-switch use claude-glm

# 切回 Anthropic 官方 OAuth（不再需要重新登录）
ha-switch use claude-anthropic-work

# Codex 当前是 ChatGPT OAuth 登录，捕获为 profile (oauth_capture)
ha-switch import-current codex chatgpt-work
...
- 通用状态管理框架。
- 系统代理、Git 身份、Shell 环境变量、服务进程等系统级集成。core 的抽象不排斥这些场景，但 MVP 不提供内置 Definition、专用 handler 或安全策略。
...
- 登录、重新登录、引导登录或修复登录状态。凭据失效时，用户在目标 App 外部自行处理，ha-switch 只负责重新捕获当前本地状态。
- 校验账号凭据在服务端是否仍可用。
- GUI 和 TUI。
- Linux Secret Service、Windows Credential Manager backend。MVP 仅实现 macOS Keychain backend、跨平台文件源，以及 Windows release 二进制；Windows Credential Manager 不进入 MVP。
...
- **失效边界**：refresh 失败时不假装成功，也不提供登录修复流程。用户应在目标 App 外部恢复可用状态后，再用 `ha-switch import-current` 捕获或更新 profile。
...
用户可以通过 `~/.ha-switch/apps.d/*.yaml` 增加新的 App Definition，也可以通过 override 文件调整系统预置 App 的默认字段、模型和 provider 模板。App id 全局唯一，必须满足与 profile id 相同的 slug 规则。

### 4.2 App Definition

App Definition 是 ha-switch 对某个本地 App / CLI 状态面的产品知识声明。它不保存用户 profile 数据，也不保存 secret。它只描述：
...
2. 用户 App Definition：`~/.ha-switch/apps.d/*.yaml`。
3. 用户 override：`~/.ha-switch/overrides.d/*.yaml`。

同一个 app id 的 override 只能修改允许覆盖的声明式字段，不能替换 handler 为未知值，不能声明执行脚本，不能扩大到 home 目录外的写入目标。

系统预置 Definition 本身也使用同一套 YAML schema。源码中放在 `src/app_definitions/builtin/*.yaml`，构建时扫描该目录、校验并嵌入二进制。发布包可以额外安装一份只读副本到资源目录（例如 macOS app bundle resources、Homebrew cellar 或 Linux `/usr/share/ha-switch/app_definitions/`），仅用于审计和调试；运行时以二进制内嵌版本为准，避免资源文件缺失导致内置 App 不可用。

用户想调整内置 Definition 时，不直接修改只读副本，而是在 `overrides.d/` 写局部 override。为了让用户从内置内容开始，CLI 提供导出命令：

```bash
# 查看完整 resolved definition
ha-switch apps show claude

# 导出系统内置原始 definition 到 stdout
ha-switch apps export claude --source system

# 生成一个可编辑的 override 起点
ha-switch apps export claude --as override --output ~/.ha-switch/overrides.d/claude.yaml
```

`--source system` 输出二进制内嵌的系统 Definition，不包含用户 override；`ha-switch apps show` 输出合并后的 resolved Definition；`--as override` 只输出允许覆盖字段的骨架和注释，避免用户复制整份系统 Definition 后误以为可以替换 handler、target path 或声明脚本。
...
blob 内容不内联到 profiles.yaml，而在 `~/.ha-switch/captures/<id>/` 下，目录 `0700`、文件 `0600`。
...
- Claude `oauth_capture` target：Keychain entry + `~/.claude.json` 的 `$.oauthAccount` 子树；如果 `~/.claude/settings.json` 中存在会覆盖 OAuth 的 managed env keys，同一次切换还要清理这些键。顶层 `userID` 不作为认证身份或 managed subtree，切换时保留现状。
...
ha-switch 的所有文件统一放在 `~/.ha-switch/` 下，跨 Linux / macOS / Windows 走同一路径，不走 XDG 三段。可通过 `HA_SWITCH_HOME` 环境变量重定向根目录（必须为绝对路径且展开后落在用户 home 内），主要用途是自动化测试以及希望走 XDG 的 Linux 用户软链到 `$XDG_CONFIG_HOME/ha-switch`。MVP 不做更精细的 XDG split。

系统预置 App Definition 随二进制发布。权威版本在源码的 `src/app_definitions/builtin/*.yaml`，构建时嵌入二进制；安装包可同时放一份只读资源副本供用户查看。它们不落在 `~/.ha-switch/` 下。用户目录只保存扩展、覆盖、profile、capture、备份和运行时状态。
...
~/.ha-switch/
  ...
  state/                           # 运行时状态，工具自动维护，不应手编
    ...
    pending-switch/                # apply/bookkeeping 之间的崩溃恢复 journal；正常情况下为空
      claude.json
    edit/                          # ha-switch edit 的临时片段目录；0700，片段文件 0600，用后删除
    history.jsonl                  # 操作历史
  locks/                           # 文件锁
    claude.lock
    codex.lock
...
ha-switch 把配置和状态分散在若干文件 / 目录里，分别承担清晰不同的职责：
...
| `~/.ha-switch/apps.d/*.yaml` | 用户新增的 App Definition（系统未预置的 App）| 用户手编 |
| `~/.ha-switch/overrides.d/*.yaml` | 对已有 App Definition 的局部覆盖（白名单字段）| 用户手编 |
| `~/.ha-switch/profiles.yaml` | **profile 注册表 + CLI 偏好** | 用户手编；ha-switch 只在 add / edit / remove / import-current 等 profile 管理命令中写入 |
| `~/.ha-switch/captures/<id>/` | oauth_capture profile 的 blob 内容和 manifest | ha-switch 自动维护（import-current / writeback）|
| `~/.ha-switch/backups/<app>/<ts>/` | 防御性备份 | ha-switch 在写操作前自动创建 |
| `~/.ha-switch/state/active.json` | 每个 App 当前活动 profile id | ha-switch 在 use 成功、remove / detach 清空活动指针时更新；restore-target 永不更新 |
| `~/.ha-switch/state/pending-switch/<app>.json` | apply 与 bookkeeping 之间的崩溃恢复 journal | ha-switch 在写目标前创建，成功提交或恢复后删除 |
| `~/.ha-switch/state/history.jsonl` | 操作历史元数据（append-only）| ha-switch 追加写 |
| `~/.ha-switch/locks/` | 文件锁 | ha-switch 临时持有 |

#### profiles.yaml 的职责和写入边界

profiles.yaml 是用户可手编的稳定配置。ha-switch 可以读取它，但只有明确修改 profile 注册表的命令可以写它：`ha-switch add`、`ha-switch edit`、`ha-switch remove`、`ha-switch import-current`（以及未来同类 profile 管理命令）。`ha-switch use`、`ha-switch status`、`ha-switch doctor`、`ha-switch restore-target` 和 OAuth writeback 都不得修改 profiles.yaml。

1. **Profile 注册表**：记录这台机器上有哪些 profile（id / app / kind / name / created_at 等）。无论什么 kind 都需要这层 metadata；`ha-switch list` / `ha-switch use <id>` / `ha-switch status` 都从这里读起。
...
3. **动态凭据的清单**（oauth_capture）：secret blob **不**在 profiles.yaml 里。这里只放：
   ...
   实际的 OAuth blob 在 `~/.ha-switch/captures/<id>/` 下作为独立文件存。`sha256`、`captured_at`、`last_writeback_at` 等动态元数据写入 `captures/<id>/manifest.json`，因此 oauth_capture 切换和 writeback 不会污染用户可编辑的 profiles.yaml。

4. **CLI 偏好**：`preferences` 块（`default_app` / `keep_backups` / `oauth_stale_warn_days`）。高风险写入确认、secret 脱敏与 secret argv 拒绝是硬性安全约束，不是 preference，不可通过配置关闭（详见 §11.1）。
...
- 代价是 profiles.yaml 通常含明文 secret，**不能直接 git commit 或公开分享，也不应放入会同步到云端的目录（iCloud Drive / Dropbox / OneDrive）或自动备份工具（Time Machine 网络备份、第三方 backup agent）的覆盖范围**。§6.7 / §11.2 强制 `0600` 权限。如果用户有跨机器同步需求，留给 Phase 3 的 `ha-switch export --unsafe-export` 提供脱敏选项。
- `ha-switch doctor` 在默认输出中必须包含一条 "profiles.yaml secret-leak surface" 提示：列出 `~/.ha-switch/` 是否位于已知云同步根目录（macOS：`~/Library/Mobile Documents/`、`~/Dropbox/`、`~/OneDrive/`、`~/Google Drive/`；Linux：用户家目录下的同名子目录）之内，并在命中时升级为 warning，引导用户把 `HA_SWITCH_HOME` 重定向到非同步目录。
...
- 同理，`~/.ha-switch/captures/<id>/manifest.json` 是工具维护的运行时索引，用户不应手编；丢失时可由 capture 文件重新计算 hash，但 `captured_at` / `last_writeback_at` 审计信息会丢失。
...
- target path 默认必须在当前用户 home 内，且经过 `~` / `${MACOS_USER}` / `${VAR:-default}` 路径环境变量展开后再做边界检查。Definition 管理的 target / capture source 路径不得位于 ha-switch 自己的配置根目录（`HA_SWITCH_HOME` 展开后的真实路径，默认 `~/.ha-switch`）之下；否则加载失败，避免用户 Definition 覆盖 `profiles.yaml`、captures、backups、state 或 locks。
...
- 只读资源目录中的系统 Definition 副本不参与运行时加载；`ha-switch apps show <app>` 可显示 resolved Definition，并标明 system 部分来自二进制内嵌 registry，必要时显示资源副本路径供用户审计。
- `ha-switch apps export <app> --as override` 生成的文件必须能被 `ha-switch apps validate` 直接通过；如果目标文件已存在，默认拒绝覆盖，除非显式 `--force`。
...
- `id` 必须满足 `^[a-z0-9][a-z0-9-]{0,63}$`。`ha-switch add` / `import-current` 自动生成的 id 形如 `<app>-<slug(name)>`，可用 `--id` 显式指定覆盖。
...
`~/.ha-switch/` 根目录及所有子目录强制 `0700`；`profiles.yaml`、用户 App Definition、override 文件、captures / backups / state 下的所有文件强制 `0600`。完整权限矩阵见 §11.2。启动时若权限被改宽，doctor 警告并提示修复。
...
| 命令 | 说明 |
|------|------|
| `ha-switch apps` | 列出已加载 App Definition、来源、支持的 kind |
| `ha-switch apps show <app>` | 展示 resolved App Definition（脱敏，不含 profile 数据）|
| `ha-switch apps export <app> [--source system\|resolved] [--as override] [--output <path>]` | 导出内置 / resolved Definition，或生成 override 起点 |
| `ha-switch apps validate [<path>]` | 校验用户 App Definition / override 文件 |
| `ha-switch list [<app>]` | 列出已注册 profile（脱敏） |
| `ha-switch show <id>` | 查看单个 profile 的元数据、verification identity、非敏感字段 |
| `ha-switch add <app> <name> [--id <id>] [--kind <kind>] [--field k=v ...] [--secret-field k=@stdin\|@prompt\|@env:NAME\|@file:PATH ...]` | 手动添加 env_injection / file_template profile |
| `ha-switch edit <id>` | 用 `$EDITOR` 打开 profile 的 yaml 片段编辑 |
| `ha-switch remove <id>` | 删除 profile；同时清理 captures/<id>/；若该 profile 是某 App 的活动 profile，将该 App 在 state/active.json 中置为 null（不动目标文件）。交互式 TTY 输入 `yes` 确认，脚本 / CI 使用 `--yes` |
| `ha-switch import-current <app> <name> [--id <id>] [--kind auto\|<kind>]` | 从 App 当前状态自动识别 kind 并捕获 profile |
| `ha-switch use <id>` | 切换到指定 profile（oauth_capture 会先写回当前活动 profile 的 capture，不修改 profiles.yaml）。当 `<id>` 就是当前活动 oauth_capture profile 自身时，命令退化为"只跑 writeback"——**仅更新 capture bytes + manifest，不更新 profile metadata / identity 字段**。若用户希望同时刷新 identity（例如 Anthropic 在 web 后台改了 `organizationName` / `subscriptionType`），应使用 `ha-switch import-current <app> <name>`，详见 §8.2 / §8.3 short-circuit。`use` 本身需要确认：交互式 TTY 输入 `yes`，脚本 / CI 使用 `--yes` |
| `ha-switch use <id> --dry-run` | 只打印 plan，不写入 |
| `ha-switch detach <app>` | 把 `state/active.json` 中 `<app>` 的活动 profile 置 null，不动 live target、不动 captures、不动 profiles.yaml；用于 `status` 报 `drifted` 后用户已在 App 外部自行处理状态、希望停止 ha-switch 对该 App 的追踪 |
| `ha-switch status [<app>]` | 显示每个 App 的活动 profile、是否 drift |
| `ha-switch backup list [<app>]` | 列出防御性备份 |
| `ha-switch restore-target <app> <backup-id>` | 从防御性备份恢复目标位置。交互式 TTY 输入 `yes` 确认，脚本 / CI 使用 `--yes` |
| `ha-switch doctor [<app>]` | 检查路径、权限、字段完整性、当前平台 capture source 存在性、进程状态、Keychain 可访问性 |
| `ha-switch config path` | 打印 profiles.yaml 路径 |

### 7.2 命令选项约定

- `--yes` / `-y`：高风险写入或安全逃生口的非交互确认开关；不跳过 identity、backup、lock、path、schema、secret redaction 等安全检查。`use`、`restore-target`、`remove` 和 `--assume-app-stopped` 逃生流程必须显式确认：交互式 TTY 中可省略 `--yes` 并在 prompt 中输入 `yes`，非交互环境必须传 `--yes`。`add` 和普通 `import-current` 表达的是创建 / 捕获当前状态，不接受 `--yes`；`import-current --yes` 仅在同时传入 `--assume-app-stopped` 时合法。
- `--json`：以 JSON 格式输出（同样脱敏）。`--dry-run` 与 `--json` 组合时，plan 以 JSON 输出（字段集合与人类格式一致，secret / blob 字段同样以 `"***"` 占位），便于脚本化校验和 MVP 验收。
- `--allow-running`：在目标 App 进程运行时仍允许纯 env_injection / file_template 写操作继续。**对任何涉及 `oauth_capture` 读写的操作不生效**——包括切到 OAuth profile、从当前 active OAuth profile 切走时的 writeback、`ha-switch use <当前 OAuth profile>` 的 writeback-only 操作、`import-current` 创建或刷新 OAuth profile，以及 `restore-target` 恢复 manifest 中 `requires_app_stopped: true` 的 target；这些场景永远要求 App 退出，除非使用 §11.4 的 `--assume-app-stopped` 逃生流程。
- `--assume-app-stopped`：仅用于涉及 `oauth_capture` 读写的操作发生进程探测命中、但用户确认为误报时的逃生口（见 §11.4）。不能作为预防性默认参数使用；如果没有进程探测命中，或当前操作不是 OAuth / process-sensitive 写入，CLI 必须报 `InvalidSafetyFlag` 并要求用户去掉该参数或改用 `--allow-running`。必须显式拼写完整，没有短形式；必须通过 `--yes` 或交互式 prompt 确认。触发时会把探测到的 PID 列表写入 `history.jsonl` 的 `warnings`。
- `--force`：仅用于明确覆盖已有输出或已有 profile 的命令，例如 `add` 同 id 覆盖和 `apps export --output` 覆盖现有文件；不作为写操作确认的别名。
...
- `--secret-field k=@stdin|@prompt|@env:NAME|@file:PATH` 用于敏感字段：`@prompt` 为 TTY masked prompt，`@stdin` 从标准输入读取一次，`@env:NAME` 只把环境变量名放进 argv、值从进程环境读取，`@file:PATH` 从本机文件读取且同样做 home 边界和权限检查。
- MVP 在交互式 TTY 中弹出确认 prompt，要求用户输入完整 `yes`；在非交互环境中不尝试读取确认，必须传 `--yes`。需要确认的操作包括覆盖目标 App 状态、恢复备份、删除状态或绕过进程安全判断。`add` 和普通 `import-current` 不接受 `--yes`；`edit` 的确认动作是用户保存编辑器内容。

### 7.3 暂缓命令

不进入 MVP：

- `ha-switch rename`（用 `remove` + `add` 替代）。
- `ha-switch diff` / `plan` / `apply` 通用三件套。
- `ha-switch backup prune`（MVP 自动按 `keep_backups` 修剪）。
- `ha-switch export` / `import` 跨机器迁移。
- shell completion 脚本。
- `ha-switch module install`、`ha-switch plugin`、`ha-switch trust` 等可执行插件 / 沙箱概念。
...
- `ha-switch add claude glm` → `claude-glm`
- `ha-switch add claude "GLM 智谱"` → `claude-glm`（中文字符在 slug 中被丢弃，详见 §6.5）
- `ha-switch add codex personal --id corp-prod` → `corp-prod`（显式 `--id` 覆盖自动规则）

不修改任何目标位置。

### 8.1.1 `ha-switch edit <id>`
...
write editable YAML fragment to ~/.ha-switch/state/edit/<operation_id>.yaml (0600)
...
`edit` 不修改 live target、captures 或 active pointer。编辑片段可能包含 env_injection / file_template 的静态 secret，因此临时片段目录必须位于 `HA_SWITCH_HOME` 下并继承 §11.2 的权限约束；不得使用系统默认临时目录。若用户编辑的是当前 active profile 的字段，命令成功后 `status` 可能显示 `drifted`，直到用户重新 `ha-switch use <id>` 把新意图应用到目标 App。

### 8.2 `ha-switch import-current <app> <name>`
...
- `ImportAmbiguous` 展示候选摘要和后续建议；用户必须显式重试并传入 `--kind <kind>`。
...
definition-driven import probes current state:
  ...
  - multiple modes co-exist                       -> ImportAmbiguous with candidate summary; user retries with --kind
...
for oauth_capture drafts:
  ...
  check verify: required identity fields against existing profiles:
    - if requested id already exists, refresh that profile only if the required
      identity matches; otherwise fail with ProfileExists
    - if another oauth_capture profile for the same App has matching required
      identity, refresh that existing profile instead of creating a duplicate
  on refresh:
...
ordinary import-current does not accept --yes; --assume-app-stopped requires --yes or an interactive yes prompt
...
特例：Claude 在 macOS 上检测到 Keychain `Claude Code-credentials` 时，必须把它和 `~/.claude.json` 的 `$.oauthAccount` 一起捕获——只捕获其中一个会让记录处于不一致状态。顶层 `userID` 的认证语义没有官方文档定义，MVP 保留现状，不捕获、不回放。

### 8.3 `ha-switch use <id>`
...
# 用户若想同步这些字段，应改用 `ha-switch import-current` 走 refresh 分支（§8.2），
...
if state.active_profiles[app] != null and target_profile.id == state.active_profiles[app].id:
    if target_profile.kind == oauth_capture:
        ...
        require confirmation (--yes or interactive yes prompt)
        run Step B only
...
| `matched-with-overrides` | ha-switch 管理的 target 与 active profile 一致，但检测到更高优先级的外部配置、环境变量或 managed policy 可能覆盖实际运行认证 |
...
### 8.5 `ha-switch restore-target <app> <backup-id>`
...
`ha-switch list` / `show` 的流程平凡，略。

### 8.6 `ha-switch remove <id>`
...
注意：remove 只清理 ha-switch 自身的注册表和 capture，不还原目标 App 当前的本地状态——用户如果希望"删掉 profile 同时把目标文件清空到无配置状态"，应先 `ha-switch restore-target` 到一个早期备份，再 `ha-switch remove`。

### 8.7 `ha-switch detach <app>`

`detach` 是 `drifted` 状态后的官方恢复路径：用户已经在 App 外部把本地登录状态改成了 ha-switch 不再追踪的样子（比如 `claude logout`、用 web 登录别的账号、手动改 settings.json 等），现在希望明确告诉 ha-switch 停止把任何 profile 标为该 App 的活动 profile，但既不删除原 profile（capture 可能还想留作历史），也不动 live target（用户可能正在用现在的状态）。
...
`detach` 之后的推荐下一步**默认是 `ha-switch import-current`，而不是 `ha-switch use`**。理由：`detach` 多半发生在 `status` 报 `drifted` 之后，意味着 live 状态已经偏离 ha-switch 的认知；此时如果直接 `ha-switch use <旧 profile>`，会用 capture 中的旧 bytes（含很可能已被 server 端旋转作废的 refresh_token）覆盖当前 live 状态，且因为没有"当前活动 profile"可 writeback，旧 capture 也不会得到一次刷新机会。`detach` 成功输出和 `status <app>` 在 `no-active` 状态下都必须打印这条提示：

```text
claude is now detached. To capture the current state as a new or refreshed
profile, run:
  ha-switch import-current claude <name>
If you instead want to roll the live state back to an existing profile, run
`ha-switch use <id>` — note this will overwrite live state with a (possibly
stale) capture and will NOT writeback first.
```

技术上 `detach` 之后两条路径都允许：

- **推荐**：`ha-switch import-current <app> <new-name>` 把当前 live state 捕获成新 profile，或在 required identity 与已有 profile 重合时刷新现有 capture（参见 §8.2）。
- **次选**：`ha-switch use <existing-id>` 切回任意已注册 profile；该次 `use` 因 active 为 null，**不执行 writeback**（没有"当前活动 profile"可写回），但仍走完整的 defensive backup → apply → verify → bookkeeping。
...
- 展开后的 target / capture source 路径还必须落在 ha-switch 配置根目录之外。`HA_SWITCH_HOME` 先按同样规则展开、规范化和 symlink 解析；任何 managed target、capture source 或用户 Definition 声明的写入路径如果等于该根目录或位于其子路径下，加载或写命令必须拒绝。`~/.ha-switch` 只允许由 ha-switch 内部配置 / 状态代码访问，不能作为 App Definition 的 target。
...
```text
~/.ha-switch/locks/<app>.lock     # 单 App 的状态写操作
~/.ha-switch/locks/profiles.lock    # profiles.yaml 写操作
~/.ha-switch/locks/state.lock       # active.json / history.jsonl 共享状态写操作
~/.ha-switch/locks/target-<sha256>.lock  # 规范化 target id 的跨 App 锁
```
...
锁实现使用 POSIX advisory lock（`flock(2)` 或 `fcntl(F_SETLK)`），持有者状态由 OS 维护——ha-switch 进程崩溃、被 kill 或异常退出时，OS 自动释放锁。锁文件本身允许残留为空文件，不需要 stale lock 清理逻辑，也不在锁文件中写 PID。

注意：锁只在 ha-switch 自身实例之间生效。OAuth capture I/O 已通过"拒绝目标 App 运行时执行"额外保护。不同 App 只有在 resolved target id 集合不重叠时才真正并行；target 重叠时必须串行。
...
2. 按 App Definition 声明的 `managed_json_subtrees`（如 `[$.oauthAccount]`），用 capture 中的对应值整体替换。
...
- **Windows**：官方文档说明默认在 `%USERPROFILE%\.claude\.credentials.json`，并且设置 `CLAUDE_CONFIG_DIR` 时 credentials 文件位于该目录下。MVP 产出 Windows release，并支持通用文件 / JSON / TOML handler；但不实现 Windows Credential Manager backend，内置 Claude OAuth file source 先限定 Linux，Claude Windows OAuth capture 需要 release-candidate 实测确认后再在 Definition 中打开。
...
- 顶层 `userID`：官方文档未定义其认证语义；社区逆向信息显示它可作为 `/buddy` seed fallback。MVP 不把它作为 identity 或 managed OAuth subtree。

切换 OAuth 账号必须**同时**改 Keychain 和 `$.oauthAccount`。仅改 Keychain 会导致 UI 显示与实际身份不一致的中间态。
...
- target / capture sources：
  ...
  - Linux：`${CLAUDE_CONFIG_DIR:-~/.claude}/.credentials.json`（resolved path 必须落在 home 内；内置 Definition 的 file source 显式标记 `platforms: [linux]`，避免把 Linux capture bytes 当成 macOS Keychain 或未实测 Windows bytes 回放）
  - 所有平台：`~/.claude.json` 的 `$.oauthAccount`（json_subtree）
  - 所有平台：`~/.claude/settings.json` 的 `$.env` managed keys 清理（仅作为切到 OAuth 时的 target，不作为 OAuth capture source）
- `managed_json_subtrees`：`[$.oauthAccount]` on `~/.claude.json`。
...
- writeback 身份门禁：
  ...
  - Claude Code 2.1.150 的 macOS Keychain value 中 `claudeAiOauth.accessToken` / `refreshToken` 是 opaque `sk-ant-*` token，不包含可验证的账号 / 组织标识；Keychain bytes 只能作为 credential source 捕获和回放，不能作为 identity source。
  - 如果未来某个平台的 credential source 能声明式提取同一账号 / 组织标识，则必须与 `oauthAccount` 和 profile identity 三方一致后才允许 writeback。
  - 如果 Definition 标记为 required 的任一 live identity 缺失 / 解码失败 / 不一致，拒绝 writeback 并报 `DriftBeforeWriteback`；不得把不一致的 live state 回写进当前 profile capture。
- identity 字段必选 / 可选标注：
...
- 不能与 env_injection 同时活动：如果 `settings.json.env` 设了 `ANTHROPIC_AUTH_TOKEN`，会**覆盖** OAuth 走 API key。切到 oauth_capture 时如果发现 Claude managed env keys，plan 必须展示"将清空这些 env 键"，并把 `settings.json` 纳入 defensive backup / apply / rollback；执行写入要求显式 `--yes`。
...
4. 两者都存在 → `ImportAmbiguous`；用户必须通过 `--kind` 选择要捕获的 kind。
5. 仅 oauth_capture 候选时，自动捕获 oauthAccount，提取 identity，并按传入 name 创建 / 刷新 profile。

**doctor / status 输出**：settings.json 存在性、`managed_env_keys` 填充状态、当前平台适用的 capture source（例如 `keychain.json` / `credentials.json`）是否存在或可读、`~/.claude.json` 中 `oauthAccount.accountUuid` / `email`、Keychain entry 是否可读、是否检测到 Claude Code 进程。对 oauth_capture，status 必须执行与 writeback 身份门禁同一组 source-consistency checks：Claude Code 2.1.150 的 credential token 是 opaque `sk-ant-*`，因此 required identity 只从 `oauthAccount` 与 active profile 比对；如果两者不一致，报告 `drifted`。只有纯 existence 检查不足以支持 `matched`。
...
2. 如果 `cli_auth_credentials_store` 未显式设置且 `auth.json` 存在，**还需进一步确认 file store 是当前真实在用的 store**：`auth.json` 必须匹配一个明确形态。`auth_mode == "chatgpt"` 且存在 OAuth token 字段时识别为 ChatGPT OAuth；`auth_mode == "apikey"` 且 `OPENAI_API_KEY` 为非空字符串时识别为 API-key；兼容旧版 API-key 文件中 `auth_mode` 缺失但 `OPENAI_API_KEY` 为非空字符串、且不存在 OAuth token 字段的形态。如果这些规则都不匹配，则 `auth.json` 可能是切到 OS keyring 后未清理的残留或用户手工写坏的文件——此时 `import-current` 报 `ImportAmbiguous`，明确提示"当前未能确认 Codex 使用 file-backed credential store，请在 Codex 外部确认登录状态后重试"，**不要 silent 选 file-backed**。
...
- 目标文件：
  - `${CODEX_HOME:-~/.codex}/auth.json`：API-key 模式写入 `auth_mode: "apikey"` + `OPENAI_API_KEY` 两个字符串字段。导入时兼容旧版/手工 API-key 文件缺少 `auth_mode` 的形态，但必须要求 `OPENAI_API_KEY` 为非空字符串且不存在 OAuth token 字段。已知 ChatGPT auth 形态会在根对象保留 `OPENAI_API_KEY: null`，因此判断 API-key 模式不能只看 key 是否存在。
...
- 读 `auth.json`：
  ...
  - `OPENAI_API_KEY` 为非空字符串，且 `auth_mode == "apikey"` 或不存在 OAuth token 字段 → draft `file_template`。这兼容缺少 `auth_mode` 的 legacy API-key 文件。
...
同样默认不把 secret 接受为命令行参数值。CLI 生成或修改静态 secret 字段时必须使用 masked prompt、stdin、环境变量名或本机文件引用读取 secret；如果用户尝试 `--field api_key=sk-...` 或类似敏感字段 argv 输入，命令失败并提示改用 `--secret-field`。这是为了避免 shell history、进程列表和终端日志泄漏。

macOS OAuth capture 的 Keychain entry 会被复制到 `~/.ha-switch/captures/` 和 `backups/` 的 `0600` 文件中。MVP 明确接受这个本地明文存储取舍以换取离线切换能力，但输出、plan、history 不得打印其内容；Phase 3/4 再引入字段级加密或 Keychain-per-profile 存储。

### 11.2 文件权限强制

- `~/.ha-switch/` 根目录：`0700`。
- `~/.ha-switch/profiles.yaml`：`0600`。
- `~/.ha-switch/apps.d/*.yaml`、`~/.ha-switch/overrides.d/*.yaml`：`0600`。
- `~/.ha-switch/apps.d/`、`~/.ha-switch/overrides.d/`：目录 `0700`。
- `~/.ha-switch/captures/`：目录 `0700`，文件 `0600`。
- `~/.ha-switch/backups/`：目录 `0700`，文件 `0600`。
- `~/.ha-switch/state/`：目录 `0700`，文件 `0600`。
- `~/.ha-switch/locks/`：目录 `0700`。
- 目标文件继承现有权限，新建默认 `0600`。
- Keychain entry：依赖 macOS Keychain 自身权限模型；ha-switch 只用当前用户 login keychain。

启动时若发现 `~/.ha-switch/` 任一文件 / 目录权限被改宽，doctor 警告并提示修复。
...
**doctor 报告**：`ha-switch doctor [<app>]` 输出每个 App 的：
...
- **任何涉及 oauth_capture 读写的操作：命中默认强制拒绝，`--allow-running` 不生效**。这包括切到 OAuth profile、从当前 active OAuth profile 切走时的 writeback、`ha-switch use <当前 OAuth profile>` 的 writeback-only 操作、`import-current` 创建或刷新 OAuth profile，以及 `restore-target` 恢复 manifest 中 `requires_app_stopped: true` 的 target。理由：App 运行时会刷新 token，原子性无法保证，且我们读取、写回或恢复的瞬间 App 也可能在写。
...
漏报方向无法用 CLI flag 防御——这是 App Definition `process_probe` 表达力的限制。第一类用户教育：任何涉及 oauth_capture 读写的操作前，用户应主动退出目标 App（例如 GUI Quit 或结束对应 CLI 进程）并通过 `ha-switch doctor <app>` 二次确认；用户必须理解 §11.4 默认拒绝是"对漏报偏保守"的设计选择，而非额外强制保证。Phase 2 可考虑给 `process_probe` 增加 `lockfile` / `socket` 等次级 handler 来收紧漏报。
...
2. **失效边界**：如果 capture 长期不被切到（即 refresh_token 已老化），用户切过去时 App 会在首次 refresh 时失败。CLI 基于 capture manifest 的 `last_writeback_at ?? captured_at` 提前警告，但不提供登录修复能力；用户在目标 App 外部恢复可用状态后，再运行 `ha-switch import-current` 捕获或更新 profile。
...
| `ImportAmbiguous` | `import_current --kind auto` 检测到多种可能 kind；用户需显式重试并传入 `--kind` |
| `LockBusy` | 另一个写操作正在执行 |
| `AppRunning` | 目标 App 进程在运行；任何涉及 oauth_capture 读写的操作默认拒绝（仅 `--assume-app-stopped` 加 `--yes` 或交互式确认可逃生），纯 env_injection / file_template 操作默认拒绝（可 `--allow-running` 跳过）；错误信息含探测到的 PID + 命令行 + 启动时间 |
...
next: ha-switch restore-target claude 20260523T100000Z
hint: target profile's capture may be from a different Anthropic account.
      fix the current App state outside ha-switch, then run
      `ha-switch import-current claude <name>` to capture or update it.
```
...
当前实现已经纳入 M1.5 的 pending-switch 自动补提交 / 自动回滚、`restore-target` 崩溃恢复、并发锁顺序和 state lock 约束；因此 M1 中曾允许的"只检测 + 拒绝"只是历史拆分口径，不再是当前发布候选的行为边界。发布判断以 §13 和 `docs/acceptance.md` 的当前证据为准。
...
- 进程互斥（含 `--assume-app-stopped` 逃生口）、secret argv 拒绝、脱敏输出、路径边界、`HA_SWITCH_HOME` 隔离。
- 用户声明式 App Definition 的加载、校验、override 合并、`apps` 子命令族。
- pending-switch journal 的存在必须阻止新的同 App 写操作直接开始；当前实现会在可证明 live state 已到达目标或仍等于 rollback backup 时自动补提交 / 自动回滚，否则拒绝并提示人工恢复。

**M1.5 — 并发与崩溃恢复加固（当前实现已纳入）**（在 M1 之上叠加 §13 第 23、24、25、26、32、33、34、35、36、37、39 条）
...
- 用户 Definition 路径与 `HA_SWITCH_HOME` 边界的完整校验矩阵——M1 仅做最小必要校验（绝对路径 + home 内 + 非 `HA_SWITCH_HOME` 子路径）。
- 跨 App target lock 在真实并发负载下的性能与公平性测试。

里程碑之间的功能 / 安全约束是**单调累加**的：M1 已经实现的安全护栏（OAuth 进程互斥、secret 脱敏、home 边界等）不允许在 M1.5 / M2 中放宽。

发布证据按 `docs/acceptance.md` 的当前阶段口径执行：当前 macOS-evidenced stage release 只把 §13 第 2 项的真实 macOS Claude OAuth import 作为 release blocker；§13 第 5 项中更广的重启验证、Linux / Windows 真实 App 验证，以及 §13 末尾 A–E 项中未形成 durable release evidence 的实验，允许作为 deferred follow-up 追踪。只要存在 deferred evidence，README、release notes 和 acceptance 报告都不得宣称 full §13 coverage。
...
4. 能用 `import-current --kind auto` 从 file-backed Codex `${CODEX_HOME:-~/.codex}/auth.json` 识别 API-key 模式（`auth_mode == "apikey"` + `OPENAI_API_KEY`，或 legacy `OPENAI_API_KEY` only 且无 OAuth token 字段）→ `file_template`，或 ChatGPT OAuth 模式（`auth_mode == "chatgpt"` + `tokens.refresh_token`）→ `oauth_capture`；Codex keyring credential store 在 MVP 中明确报 `CredentialStoreUnsupported`。
5. 能在已注册 profile 之间切换：
   ...
   - 切换后重启 App 实测确认 profile 生效（macOS、Linux、Windows 按该平台支持的 app/profile kind 各做一次；Windows Claude OAuth capture 在 Definition 打开前只做可用 kind 和 release smoke）。
6. `ha-switch use` 只要当前活动 profile 是 oauth_capture 且需要 writeback（包括切到 oauth_capture B、切到 env/file profile、以及 `ha-switch use <当前 OAuth profile>` 的 writeback-only 操作），都必须先校验 live target identity 仍等于当前活动 oauth_capture A，并执行 Definition 声明的 credential source / identity source 一致性检查；通过后才把 A 的最新 Keychain / 凭据文件 / json_subtree / managed TOML fragment 写回 A 的 capture，并更新 `captures/A/manifest.json`。identity 不匹配或 source 不一致时必须拒绝写回且不得修改 capture / profiles.yaml。
...
18. 能从 `~/.ha-switch/apps.d/*.yaml` 加载一个用户声明式 App Definition，并用已有 handler 完成 `env_injection` 或 `file_template` profile 的 add/use/dry-run。
19. 用户 Definition 引用未知 handler、写入 home 外路径或包含可执行脚本字段时，加载失败且写命令拒绝执行。
20. `ha-switch apps` 能展示每个 App Definition 的来源（system / user / override）和支持的 kind；`ha-switch apps show <app>` 能展示 resolved Definition；`ha-switch apps export <app> --as override` 能生成可验证的 override 起点；`ha-switch apps validate` 能校验单个 Definition 文件。
...
26. `ha-switch detach <app>` 仅把 `state/active.json` 中该 App 的活动 profile 置 null，不动 profiles.yaml、captures/、backups/ 和任何 live target；detach 后 `status` 报 `no-active`，`import-current` 与 `use` 仍可正常工作（首次 `use` 因无 active 不执行 writeback）。
...
29. 备份目录使用 hardlink dedup（文件系统支持时），`ha-switch doctor` 同时报告备份的 inode 占用与逻辑字节总和；`keep_backups` 内总大小超过 100 MB 给出 warning。
30. 用户 App Definition 的 target / capture source 路径如果落在 `HA_SWITCH_HOME`（默认 `~/.ha-switch`）之内，加载失败；写命令不得通过 Definition 覆盖 profiles.yaml、captures、backups、state 或 locks。
...
39. `doctor` 默认输出必须包含 "profiles.yaml secret-leak surface" 检查：当 `~/.ha-switch/` 位于 iCloud Drive / Dropbox / OneDrive / Google Drive 等已知云同步目录之内时升级为 warning。
40. `detach` 成功输出和 `status` 在 `no-active` 状态下必须显式打印 "推荐 `ha-switch import-current` 而非 `ha-switch use`" 的提示（含原因：`use` 不 writeback，会用可能已陈旧的 capture 覆盖 live state）。
41. Codex `import-current` 在 `cli_auth_credentials_store` 未显式设置且 `auth.json` 存在时，必须按声明式导入规则识别明确形态：ChatGPT OAuth 需要 `auth_mode == "chatgpt"` 和 OAuth token 字段；API-key 需要非空字符串 `OPENAI_API_KEY`，且没有 OAuth token 字段，`auth_mode` 可为 `"apikey"` 或缺失以兼容 legacy 文件。若 `auth.json` 不匹配任何明确形态，必须报 `ImportAmbiguous` 而非 silent 选 file-backed。

**真实 App 实测（release evidence 按 `docs/acceptance.md` 分阶段记录）**：

这些项目用于证明 full §13 coverage。当前 macOS-evidenced stage release 只要求 macOS Claude OAuth import release blocker 通过；其余项目可以 deferred，但必须在 `docs/evidence-followups.md`、release checklist 或 maintainer-visible issue 中持续追踪，且不得被公开 release notes 表述为已完成 full coverage。
...
C. **Claude 并发写 `~/.claude.json` 实测**：Claude Code 运行时观察 `~/.claude.json` 的写频率与字段，识别哪些字段会和 `$.oauthAccount` 同一次写入，并记录顶层 `userID` 是否独立变化。同时采样 Claude Code 自己写出的 JSON 格式参数（indent 字符与宽度、是否 trailing newline、是否 minified、顶层 key 顺序），把结果固化为 §9.4 采样算法的默认值或 doctor 校验项；如果 Claude Code 写出的格式与 §9.4 默认值不同，说明 ha-switch 必须严格沿用读入时的采样而不能落回默认。
D. **Codex auth.json schema 实证**：已在 Codex CLI 0.133.0（macOS）确认 ChatGPT OAuth file-backed 形态为根字段 `OPENAI_API_KEY: null`、`auth_mode: "chatgpt"`、`last_refresh`、`tokens.{access_token,account_id,id_token,refresh_token}`；API-key 模式为根字段 `auth_mode: "apikey"` + `OPENAI_API_KEY` 两个字符串。MVP 也兼容 legacy API-key 文件只包含非空字符串 `OPENAI_API_KEY` 且没有 OAuth token 字段的形态。
...
- `ha-switch rename` / `ha-switch tag` / shell completion。
- 更细的 drift 展示：diff 出具体哪些键 / 字段被外部改了。
- `ha-switch export` / `import`：跨机器迁移（明文导出需 `--unsafe-export`）。
...
如果内置场景之外出现明确需求，再考虑更通用的 state / plan / apply 模型、跨 App 组合切换、可执行插件协议、trust / allow 机制。这些能力不应反向污染第一阶段 MVP，也不改变 core 当前的基本职责：安全、可预期地把结构化 profile 应用到一组本地 targets。

## 15. 当前设计结论

`ha-switch` 第一版是一个**本地 profile / state 切换器**。Claude Code / Codex 的账号和凭据切换是 MVP 的首个内置应用域，因为它同时覆盖了静态字段渲染、OAuth 动态 capture、secret 脱敏、备份回滚和 App Definition 扩展等核心能力。
