...
这份指南说明如何把 `ha-switch` 当作日常的本机应用 profile 切换工具使用。它尽量避开实现细节；如果需要架构和安全模型，请看 `docs/design.md`。

## 核心概念

**App** 是一个可以由 `ha-switch` 管理本地状态的工具。构建产物可以包含内置 app definition，用户也可以在 `apps.d/*.yaml` 下添加更多定义。
...
`ha-switch` 不会替你登录远端服务。你先用目标应用自己的方式完成登录或配置，再用 `ha-switch` 保存和回放这些本地状态。
...
`config path` 会打印主 profile registry 的可编辑路径。`doctor` 会打印 ha-switch home 目录、`profiles.yaml` 路径、权限检查、已知云同步风险，以及 app-specific 诊断。

默认情况下，profile 和 capture 保存在 `~/.ha-switch`。这个目录可能包含静态 secret、OAuth capture 和防御性备份。不要把它放进 iCloud Drive、Dropbox、OneDrive、Google Drive 等云同步目录；`doctor` 会在检测到已知同步目录时报警。

`HA_SWITCH_HOME` 是推荐的状态目录覆盖变量。旧的 `ANY_SWITCH_HOME` 仍会作为 fallback 读取，以兼容已有脚本；如果两者同时存在，优先使用 `HA_SWITCH_HOME`。
...
Codex profile 切换只管理 Codex 的 `auth.json` 和受管 `config.toml` 字段。对话记录留在同一个 `CODEX_HOME` 下作为共享状态，不会被 profile 切换捕获、备份、恢复或删除。

先预览，再把旧 Codex home 合并到当前 Codex home：
...
迁移会追加缺失的 `history.jsonl` 和 `session_index.jsonl` 行，复制缺失的 `sessions/` 文件，跳过完全相同的文件；如果目标已经有同路径但内容不同的 session 文件，会报错并停止，不会覆盖。
...
live app identity 已经不再匹配 `ha-switch` 当前认为 active 的 profile。切换会被阻断，避免把错误的 live state 写回旧 profile。
...
写入受管理 target 前，`ha-switch` 会创建备份。
...
`restore-target` 会恢复目标应用的 live state，但不会把某个 profile 标记为 active。恢复后运行 `ha-switch status <app>`，检查恢复出的状态是否匹配 active profile。交互式终端中输入 `yes` 确认，脚本和 CI 中加 `--yes`。对于 OAuth 或进程敏感 target，restore 遵循和 switching 相同的 stop-app 规则。
...
`remove` 会删除 profile 和它的 ha-switch capture 文件。它不会清空或恢复目标应用当前的 live state。交互式终端中输入 `yes` 确认，脚本和 CI 中加 `--yes`。
...
`ha-switch` 会依次使用 `$VISUAL`、`$EDITOR`，然后回退到构建目标平台的默认编辑器。保存前会校验修改后的 profile。
