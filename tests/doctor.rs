...
fn doctor_warns_when_switch_home_is_under_cloud_sync_root() {
    ...
    let switch_home = home.path().join("Dropbox").join(".ha-switch");
    ...
    Command::cargo_bin("ha-switch")
        ...
        .success()
        .stdout(contains(
            "profiles.yaml secret-leak surface\twarning: HA_SWITCH_HOME appears to be under a cloud sync directory",
        ));
}
