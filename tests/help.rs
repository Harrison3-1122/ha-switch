...
fn help_keeps_escape_hatches_out_of_normal_examples() {
    ...
    assert!(help.contains("ha-switch import-current claude work --kind oauth_capture"));
    assert!(help.contains("ha-switch use claude-work"));
    assert!(!help.contains("ha-switch use claude-work --assume-app-stopped"));

    let use_help = command_help(&["use", "--help"]);
    assert!(use_help.contains("ha-switch use claude-work"));
    assert!(!use_help.contains("ha-switch use claude-work --assume-app-stopped"));

    let import_help = command_help(&["import-current", "--help"]);
    assert!(import_help.contains("ha-switch import-current claude personal --kind oauth_capture"));
    assert!(!import_help.contains(
        "ha-switch import-current claude personal --kind oauth_capture --assume-app-stopped"
    ));
    ...
    assert!(config_help.contains("ha-switch doctor"));
    ...
    assert!(backup_help.contains("ha-switch backup list <app>"));
    assert!(backup_help.contains("ha-switch restore-target <app> <backup-id>"));
    ...
    assert!(detach_help.contains("ha-switch import-current <app> <name>"));
    assert!(detach_help.contains("ha-switch import-current claude current-state"));
    assert!(detach_help.contains("does not write back first"));
}

fn command_help(args: &[&str]) -> String {
    let stdout = Command::cargo_bin("ha-switch")
        .unwrap()
        .args(args)
        .assert()
        .success()
